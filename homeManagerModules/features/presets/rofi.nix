...
with lib;
{
  ...
  config = mkIf cfg.enable {
      ...
      optional cfg.rofi-power (
        pkgs.writeShellScriptBin "rofi-power" ''
          ...
          show_menu() {
          	...
          	options="${
             if hasScreenLocker then "$lock_screen\n" else ""
           }$standby\n$hibernate\n$reboot\n$shutdown"
    ...
    systemInterface.applications.appLauncher = mkIf programCfg.enable (
      if cfg.uwsm then "rofi -show drun -run-command \"uwsm app -- {cmd}\"" else "rofi -show drun"
    );
